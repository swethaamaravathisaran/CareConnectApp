...
const BabyForm = ({ onSubmit, initialData, clearEdit }) => {
  const [formData, setFormData] = useState({ name: '', gender: '', birthDate: '', photo: '' });
  ...
  return (
    <form className="baby-form" onSubmit={handleSubmit}>
      <input type="text" name="name" placeholder="Baby Name" required value={formData.name} onChange={handleChange} />
      ...
      <input type="text" name="photo" placeholder="Photo URL (optional)" value={formData.photo} onChange={handleChange} />
      <button type="submit">{initialData ? 'Update Baby' : 'Add Baby'}</button>
      {initialData && <button type="button" onClick={clearEdit}>Cancel</button>}
    </form>
  );
};
